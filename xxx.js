...
function cargarProductos(select) {
    ...
    if (!familia || !productosSelect) return;
    ...
    fetch(`obtener_productos.php?familia=${encodeURIComponent(familia)}`)
        ...
        .then(data => {
            ...
            const opciones = data.map(p =>
                `<option value="${p.id}" data-nombre="${p.nombre}" data-precio="${p.precio}" data-descripcion="${p.descripcion}">
...
function agregarProducto(select) {
    ...
    if (existente) {
        ...
        carrito.push({ producto_id: id, nombre, descripcion, precio, cantidad: 1, subtotal: precio });
    }
