    ...
    fetch('obtener_productos.php?familia=' + familia)
        ...
        .then(data => {
            ...
            data.forEach(producto => {
                const texto = `${producto.nombre} - ${producto.marca} - ${producto.descripcion} - $${producto.precio}`;
...
function calc(valor) {
    if (valor === '.' && expresion.includes('.')) return;
    expresion += valor;
...
function calcular() {
    try {
        ...
        document.getElementById('pantalla').value = resultado.toFixed(2);
        expresion = "";
    } catch (err) {
